...
class DocumentController {
  ...
  async getUserDocuments(req, res) {
    try {
      ...
      const sharedWithUser = await SharedDocument.find({ sharedWithId: userId })
        .populate({
          path: 'documentId',
          select: '-content'
        });
  ...
  async updateDocument(req, res) {
    try {
      ...
      if (!hasWritePermission) {
        ...
        hasWritePermission = sharedWith && 
          (sharedWith.permissions === 'write' || sharedWith.permissions === 'admin');
      }
  ...
  async uploadImage(req, res) {
    try {
      ...
      if (!hasWritePermission) {
        return res.status(403).json({
          success: false,
          message: 'Você não tem permissão para adicionar imagens a este documento'
        });
      }
