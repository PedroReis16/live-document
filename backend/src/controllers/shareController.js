...
class ShareController {
    ...
    try {
      ...
      if (document.ownerId.toString() !== ownerId) {
        return res.status(403).json({
          success: false,
          message:
            "Você não tem permissão para alterar compartilhamentos deste documento",
        });
      }
  ...
  async generateShareLink(req, res) {
    try {
      ...
      const baseUrl = process.env.CLIENT_URL || "https://document-app.com";
  ...
  async joinByShareToken(req, res) {
    try {
      ...
      if (!share) {
        ...
        if (
          permissionLevels[shareLink.permission] >
          permissionLevels[share.permissions]
        ) {
      ...
      if (!isOwner) {
        ...
        if (!hasAccess) {
          return res.status(403).json({
            success: false,
            message:
              "Você não tem permissão para ver os colaboradores deste documento",
          });
        }
      }

      // Buscar o proprietário do documento
      const owner = await User.findById(document.ownerId).select(
    ...
    try {
      ...
      if (!isOwner) {
        ...
        if (!userShare) {
          return res.status(403).json({
            success: false,
            message:
              "Você não tem permissão para gerenciar colaboradores deste documento",
          });
        }
      }
      ...
      if (document.ownerId.toString() === userId) {
        return res.status(400).json({
          success: false,
          message:
            "Não é possível alterar as permissões do proprietário do documento",
        });
      }
      ...
      const collaborator = await User.findById(userId).select(
        "_id name email username avatar"
      );
